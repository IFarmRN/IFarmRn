...
function livestock(props) {
  ...
  buttonSubmitted = async () => {
    ...
    if (boll == undefined) {
      ...
      let c4 = (values["Numero_de_cabeças"] * values["Numero_de_dias_para_tratar"] * c3 * 1.1 * 0.001);
      ...
      <View style={styles.container}>
        <ScrollView style={{ flex: 1 }}>
          ...
          <DropdownList
            ...
            props={props}
          />
          <Text style={styles.title}>{`Volumoso(%): ${values["Porcentagem_volumoso"]}`}</Text>
          <Text style={styles.title}>{`Concentrado(%): ${values["Porcentagem_concentrado"]}`}</Text>
...
export default withFormik({
  ...
  validationSchema: Yup.object().shape({
    Performance_produtiva_animal: Yup.string("Erro").required("Não esqueça de preencher"),
  }),
